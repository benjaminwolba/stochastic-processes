...
Hint: Start with the definition of the expectation value $\langle \exp(tx) \rangle$ as an integral,
...
\end{equation}
%
i.e., the expectation value of the empirical mean equals the true mean. One says that $\bar{x}$ is \textit{faithful} estimator of $\mu$. Similarly, for the special case that the $x_j$ are normally distributed, $x_j\sim\mathcal{N}(\mu,\sigma^2)$, we have
%
\begin{equation}
...
\begin{comment}
so the variance adds up $\sigma_{XY}^2 = \sigma_X^2 + \sigma_Y^2$ like lengths of orthogonal vectors (generalization of the Pythagorean theorem): $||x + y||^2 = ||x||^2 + ||y||^2$ if $x \perp y$)

variance one: variance is normalized / tends to the variance of the entire sample: $Var(aX + bY) = a^2 Var(X) + b^2 Var(Y)$ 

and thus $Var((x_1 + \dots + x_n)/n) = Var((x_1 + \dots + x_n))/n^2 = n Var(x)/n^2 = Var(x)/n$
\end{comment}
...
% TODO: CONTINUE HERE
% @Ben: What to continue here? Is it a remark for the lecture? 
