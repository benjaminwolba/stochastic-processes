...
% CAUTION: 'scrpage2.sty' decomissoned 									
...
\cfoot{Stochastic Processes}
	
\addbibresource{ref.bib} 										   % integration of .bib-file

% @Benjamin Wolba: please change as you deem appropriate
...
% Benjamin: these were already defined in notebook.cls 
...
% We acknowledge comments and discussions with Andrea Auconi, Veikko Geyer.	
...
\begin{theorem}[Random Variables]
	%
...
% BMF: can this be made simpler? BMF was surprised how tricky it actually is to define the term 'independent' in mathematically correct form
% BW: I think the definition is fine, I added some intuition below. 
...
% As $||x + y||^2 = ||x||^2 + ||y||^2$ holds if $x \perp y$ with $||.||$ being the norm on $L^2$, the variance is additive for two independent random variables: $\sigma_{12}^2 = \sigma_1^2 + \sigma_2^2$. 
...
\Caveat: Note that $p(x)$ is also called \textit{probability density function}  and sometimes written as $\mathrm{PDF}(x) = p(x)$.  Also, note the physical units:  if, for example, $x$ has units of a length, $[x] = \si{\meter}$,  then the probability density $p(x)$ has units of an inverse length, $[p] = \si{\per \meter}$.  Knowing the physical units is extremely useful as it allows to tell apart different objects.

\Caveat: As a mathematical caveat, Eq.~\eqref{eq:def_P} cannot be defined for arbitrarily ``wild'' sub-sets $A \subset \R$, but only for sufficiently `nice' subsets (similar to the restriction of `nice' subsets in the axiomatic definition of probability).  Specifically, these subsets need to be measurable.  In particular, (countable) unions and intersections of intervals are measurable,  forming a so-called $\sigma$-algebra.  This technical issue is extensively studied in the mathematical sub-discipline of Measure and Integration Theory, but usually is not a concern in physical applications. For a reference, see e.g. \cite{Billingsley2012}.
...
We will later specify the expected error of such an estimate.) % TODO: add reference
...
% \subsection*{Example Weather Forecast: improve}
...
% TODO: maybe include better example later
% Benjamin Question: What should be improved for this example? 
...
% TODO: harmonize notation on probability distributions in the end
% TODO: add figure of decision tree
...
\begin{caveat}[Caveat: Why is the concept of time being used here?]
...
\begin{comment}
	Benjamin: As far as I can see this applies to all independent, identically-distributed random variables – please let me know, if I am wrong, and why. 
	
	Similarly, for the special case that the $x_j$ are normally distributed, $x_j\sim\mathcal{N}(\mu,\sigma^2)$, we have
	%
	\begin{equation}
		\left\langle \widehat{\sigma}^2 \right\rangle = \sigma^2 \quad,
	\end{equation}
	%
	i.e., the expectation values of the empirical variance equals the true variance. Even in the general case, where the $x_j$ are not normally distributed, $\widehat{\sigma}^2$ often gives a useful approximation of the true variance.
\end{comment}
...
% NOTE: BMF corrected sign error in characteristic function for normal distribution (and reverted minus sign in second cumulant that he errorneously introduced as a wrong correction
\end{equation}
%
...
% TODO: make this an advanced track that students can study on 2nd reading
...
% Type of dynamical system which unfolds probabilistically, i.e. is subject to noise.
...
% TODO: CONTINUE HERE
% @BMF: What to continue here? Is it a remark for the lecture? 
...
%
\begin{theorem}[Gaussian White Noise]
...
\begin{caveat}[Caveat: Gaussian White Noise and Mathematics]
...
\begin{caveat}[Caveat: Position-dependent diffusion coefficient]
...
\begin{theorem}[Repetition: Equipartition Theorem]
...
% TODO Suggestion: 
% We could add here a general introduction to what is an "ensemble" and how the Langevin and FP-equations delivers description on different levels, i.e. for a single realization or an entire ensemble
...
\begin{theorem}[Euler-Maruyama Scheme]
...
% Ben stopped here on Nov 2nd (to be continued)
...
\begin{caveat}[Caveat: Boundary Conditions and Functional Analysis]
...
% TODO: Needs to be elaborated further, how is this related to / an example of a Master equation
...
\chapter{Diffusion to Capture}

In this chapter we consider diffusion when there is an absorbing boundary – once a diffusing particle reaches it, there is no return. This first-time hitting model can have a variety of applications, from describing an animal's search for food, a protein binding to a site, or the ruin problem where a gambler is loosing all his money. It may also, more generally, describe chemical reactions, in particular donor-bridge-acceptor systems, ground-state recovery after photoexcitation, or a particle jumping between local minima in a potential landscape. 

\section{Mean First Passage Time and Dynkin Equation}

\subsection*{Mean First Passage Time}

An absorbing boundary introduces an characteristic time-scale to a system name the time it takes on average starting somewhere off the absorbing boundary to eventually get trapped at the absorbing boundary. Mathematically speaking, if we consider a bound domain $D$, how long will it take for a particle starting with that domain at $x_1 \in D$ to get trapped at the boundary $\partial D$? This brings us to the concept of the \textit{mean first passage time} (MFPT): The average time scale for a stochastic event, such as being trapped at a boundary, to occur for the first time. 

Let's consider one specific example: Diffusion in some potential landscape $U(x)$ as described by the Langevin equation
%
\begin{equation}
	\gamma \dot{x} = - \pdv{U}{x} + \xi(t)
\end{equation}
%
with Gaussian white noise $\xi(t)$, the initial conditions $p(x,0) = \delta(x-x_1)$, and absorbing boundary conditions $p(x_2,t) = 0$. Here, $p(x,t)$ is the processes probability distribution and its time evolution $\dot{p} = -\nabla J$ gives rise to a current $J(x,t)$, with $J(x_2,t)$ being specifically the current to the absorbing boundary. The mean first passage time for this setup is 

\begin{theorem}[Mean First Passage Time (MFPT)]
	%
	\begin{equation}
	\tau(x_2 | x_1) = \int_{-\infty}^{x_2} \dd{x} t \, p(x,t) =  \int_0^{\infty} \dd{t} t \, J(x_2,t | x_1,0)
	\end{equation}
	%
\end{theorem}

% - units: integral gives one time unit, t gives one time unit, J give one inverse time unit = one overall time unit as it should be 
% 

\subsection*{Example: MFPT for Donor-Bridge-Acceptor Systems}

Let's consider one more example from chemistry, taken from the paper 'Mean First Passage Times in Biology' by Polizzi et al. \cite{Polizzi2016}: Donor-bridge-acceptor (DBA) systems, a particular type of kinetic reaction system, where a reaction occurs form an initial donor state D, via intermediate bridge states B, to a final acceptor state A. In the simplest case, there is just a single bridge state and one can think of it as a random walker jumping between donor and bridge, before eventually getting absorbed at the acceptor. In this example, the MFPT characterizes the average time it takes to get from the initial state $D$ to the final state $A$. 

The MFPT is given by 
%
\begin{equation}
	\tau = \int_0^\infty \dd{t} t \, p_{\mathrm{fp}}(t)  
\end{equation}
%
where $p_{\mathrm{fp}}(t)$ is the probability density of the first passage to the final state $A$
%
\begin{equation}
	p_{\mathrm{fp}}(t) = \dv{t} P_{\mathrm{fp}}(t)
\end{equation}
%
and $P_{\mathrm{fp}}(t)$ is the corresponding probability of a first passage to the final state $A$, which is related to the probability $P_{1}(t) + P_{2}(t)$ to be found in either state D or B (after starting at D) at time $t$ via 
%
\begin{equation}
	P_{\mathrm{fp}}(t) = 1 - \left[ P_{1}(t) + P_{2}(t) \right]
\end{equation}
%

Doing the maths (see the paper by Polizzi et al. \cite{Polizzi2016}), one obtains for the MFPT
%
\begin{equation}
	\tau = \sum_{i \neq D} \int_0^\infty \dd{t} P_i(t) = \sum_{i \neq 3} r_i
\end{equation}
%
with $i \in \{D,B,A\}$ and where $r_i$ are the residence times for the respective states. Generalizing again from this expression one can see, that the MFPT increases with more bridge states and larger residence times. The inverse of the MFPT give the effective rate of the overall chemical reaction. 

% TODO: how is this formalism related to what was discussed before? 

\subsection*{Dynkin Equation}
...
% \subsection*{Extra: General Form of Dynkin Equation}
...
\begin{caveat}[Caveat: How to add two noise terms ]
	%
	\begin{equation*}
		\xi(t) = \xi_L(t) - \xi_R(t)
	\end{equation*}
...
%
\begin{caveat}[Caveat: Even and Odd Functions]
	%
