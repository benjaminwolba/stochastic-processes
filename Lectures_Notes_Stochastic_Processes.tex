...
The \textit{expectation value} of a function $f(x)$ with respect to $p$ is defined as
\begin{equation}
...
we write $x\sim(\mu, \sigma^2)$; 
the corresponding probability density reads
\begin{equation}
P_\mathrm{normal}(x) = \frac{1}{\sqrt{2 \pi \sigma^2}} \exp[- \frac{(x- \mu)^2}{2 \sigma^2}] = N(\mu, \sigma^2)
\end{equation}
